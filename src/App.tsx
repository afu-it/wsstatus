    ...
    <div className="min-h-screen mesh-gradient selection:bg-brand-primary/20 transition-colors duration-500 font-sans">
      ...
      <nav className="fixed top-0 left-0 w-full px-3 sm:px-4 py-2.5 sm:py-3 flex justify-between items-center z-50 bg-white/90 backdrop-blur-lg border-b border-gray-100/50 shadow-sm">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          {/* Install button - always show on mobile if not installed */}
          {!isStandalone && (
            <button
              onClick={() => {
                if (deferredPrompt) {
                  ...
                  // Show about modal with install instructions
                  setIsAboutOpen(true);
                }
              }}
              className="flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 bg-brand-primary text-white rounded-full hover:bg-brand-primary/90 transition-all text-[11px] sm:text-xs font-bold shadow-sm"
            >
